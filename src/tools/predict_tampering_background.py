...
def create_pivot(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_final = pd.merge(
        df[["tampered", "tampering", "dataset_split", "gt_keypoints", "background", "id"]],
        df_pivot,
        on="id",
    )
...
def train_predictor(
    ...
    if background is not None:
        df_final = df_final[df_final["background"] == background]
    ...
    for compare_types in [[t] for t in CompareType.SELECTION()] + [
        ...
        if validate:
            ...
            results_performance.append(
                {
                    "background": background if background else "all",
...
def main():
    ...
    # Get unique backgrounds from data
    backgrounds = df_final["background"].unique().tolist()

    all_results = []
    # Run for all backgrounds combined (None) and each individual background
    for bg in [None] + backgrounds:
        df_results = train_predictor(df_final, validate=True, gt_keypoints=False, background=bg)
